...
def read_level3_logs(patient_id: str = None) -> List[str]:
    """Read level3.log file and optionally filter by patient ID"""
    try:
        log_file_path = os.path.join(os.path.dirname(__file__), '..', '..', 'logs', 'level3.log')
        ...
        print(f"Error reading level3.log: {str(e)}")
        return []
...
def get_level3(patient_ids_request: str):
    try:
        ...
        log_data = read_level3_logs(patient_ids_request)
